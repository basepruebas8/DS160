  ...
  window.addEventListener('message', (e) => {
    try{
      ...
      if (form && section && typeof section === 'object'){
        // limpia caché previa para evitar datos de otro cliente
        try{
          sessionStorage.removeItem('ds160-personal1-state-v3');
          sessionStorage.removeItem('ds160-personal1-raw-v3');
        }catch{}

        // asigna DOM directo con prioridad al payload actual
        try{
          const givenNow = section.APP_GIVEN_NAME || data.APP_GIVEN_NAME || data.app_given_name || '';
          const surnNow  = section.APP_SURNAME     || data.APP_SURNAME     || data.app_surname     || '';
          const givenEl  = document.getElementById('APP_GIVEN_NAME');
          const surnEl   = document.getElementById('APP_SURNAME');
          const nativeEl = document.getElementById('APP_FULL_NAME_NATIVE');
          if (givenEl) givenEl.value = givenNow;
          if (surnEl)  surnEl.value  = surnNow;
          if (nativeEl) nativeEl.value = (givenNow + ' ' + surnNow).trim();
        }catch(_){}
