// personal1.js (MAYÚSCULAS aplicadas solo a esta página)
// Objetivo original + MAYÚSCULAS:
...
// - NUEVO: todos los inputs de texto/textarea de ESTA página se guardan y muestran en MAYÚSCULAS.
...
(function(){
  ...
  // ===== MAYÚSCULAS (solo en esta página) =====
  ...
  function toUpper(s){
    try { return String(s||'').toLocaleUpperCase('es-MX'); }
    catch { return String(s||'').toUpperCase(); }
  }
  function wireUpper(el){
    if (!isTextLike(el) || el.__uppercaseWired) return;
    el.__uppercaseWired = true;
    let composing = false;

    el.addEventListener('compositionstart', () => { composing = true; });
    el.addEventListener('compositionend', () => {
      composing = false;
      const v = toUpper(el.value);
    ...
    // Opcional: placeholder en mayúsculas (estético)
    if (el.placeholder){
      try { el.placeholder = toUpper(el.placeholder); } catch(_){}
    }
  }
  function scanUpperInForm(form){
    if (!form) return;
    // Estilo visual
    try{
      const s = document.createElement('style');
      s.dataset.uppercasePersonal1 = '1';
      s.textContent = `
        #${form.id} input[type="text"],
        #${form.id} input:not([type]),
        #${form.id} textarea { text-transform: uppercase; }
      `;
      document.head.appendChild(s);
    }catch{}
    form.querySelectorAll('input, textarea').forEach(wireUpper);

    // Por si se agregan nodos dinámicamente
    try{
        ...
        for (const m of muts){
          if (m.type === 'childList'){
            m.addedNodes.forEach(node=>{
              ...
              if (isTextLike(node)) wireUpper(node);
              else node.querySelectorAll?.('input, textarea').forEach(wireUpper);
            });
          }
        }
      });
  ...
  function normalizeAllToUpper(form){
    if (!form) return;
  ...
  function collectNormalized(form){
    ...
    Array.from(form.querySelectorAll('input, select, textarea')).forEach(el=>{
      ...
      else { out[k]=isTextLike(el) ? toUpper(el.value) : el.value; }
    });
    ...
    // Normalización obligatoria de nombres (MAYÚSCULAS)
    const given   = toUpper((document.getElementById('APP_GIVEN_NAME')?.value||'').trim());
    const surname = toUpper((document.getElementById('APP_SURNAME')?.value||'').trim());
    const full    = [given, surname].filter(Boolean).join(' ').trim();
    const fullNative = toUpper((document.getElementById('APP_FULL_NAME_NATIVE')?.value||'').trim());
    ...
    try{
      ...
      if (normStr){
        ...
        Array.from(form.querySelectorAll('input, select, textarea')).forEach(el=>{
          ...
          } else if (t!=='file'){ el.value=isTextLike(el)?toUpper(val):val; }
        });
      } else if (rawStr){
        ...
        Array.from(form.querySelectorAll('input, select, textarea')).forEach(el=>{
          ...
          } else if (t!=='file'){ if (k in data) el.value=isTextLike(el)?toUpper(data[k]):data[k]; }
        });
      }
      // Disparar eventos para dependientes y asegurar MAYÚSCULAS visuales
      Array.from(form.querySelectorAll('input, select, textarea')).forEach(el=>{
        el.dispatchEvent(new Event('change',{bubbles:true}));
        el.dispatchEvent(new Event('input',{bubbles:true}));
      });
      normalizeAllToUpper(form);
  ...
  safe(()=>{
    ...
    // MAYÚSCULAS en esta página
    scanUpperInForm(form);
    ...
    // Autocompletar y ocultar nombre nativo (en MAYÚSCULAS)
    ...
    if (fullNative){
      ...
      const updateFull = ()=>{
        const g=toUpper((given?.value||'').trim()); const s=toUpper((surname?.value||'').trim());
        fullNative.value = [g,s].filter(Boolean).join(' ').trim();
        fullNative.dispatchEvent(new Event('input',{bubbles:true}));
        fullNative.dispatchEvent(new Event('change',{bubbles:true}));
        mergeIntoMaster();
      };
  ...
  safe(()=>{
    ...
    nextBtn.addEventListener('click', ev=>{
      ...
      normalizeAllToUpper(form); // asegura mayúsculas antes de validar/guardar
      if (!isFormValid(form)){
  ...
  console.info('personal1.js listo (MAYÚSCULAS aplicadas).');
})();
